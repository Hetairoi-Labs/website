...
export function Footer() {
	...
	return (
		<footer className="flex min-h-svh w-full flex-col overflow-hidden bg-dark px-4 pt-16 pb-6 text-dark-foreground md:px-8 md:pt-24 md:pb-8 lg:px-16">
			<div className="flex h-full flex-1 flex-col justify-between">
				<div>
					<StaggerText className="mb-10 md:mb-16" delay={0.05} triggerOnView>
						<StaggerItem>
					...
					<StaggerText className="mb-10 md:mb-16" delay={0.1} triggerOnView>
						<StaggerItem>
							<div className="h-px w-full bg-dark-border" />
						</StaggerItem>
					</StaggerText>

					<div className="grid grid-cols-1 gap-10 md:gap-16 lg:grid-cols-12 lg:gap-8">
						<StaggerText
							className="mt-0 flex flex-col gap-6 md:mt-8 md:gap-8 lg:col-span-5"
							delay={0.1}
							triggerOnView
						>
							<StaggerItem>
						...
						>
							{navLinks.map((link) => (
								<StaggerItem key={link.name}>
									<div className="group">
										<SimpleLink
											className="block py-4 font-light text-3xl text-dark-foreground/80 no-underline transition-colors hover:no-underline group-hover:text-dark-foreground md:py-6 md:text-4xl"
											href={link.href}
											onClick={(event) => {
				...
				<StaggerText
					className="mt-8 flex flex-col items-center justify-start gap-4 border-dark-border border-t pt-6 md:flex-row md:gap-6 md:pt-8"
					delay={0.3}
					triggerOnView={false}
				>
					<StaggerItem>
